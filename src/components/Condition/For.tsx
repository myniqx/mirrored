...
export function For<T>(props: ForProps<T>) {
  ...
  let eachComponent: ReactWithProp | null = null
  let fallbackComponent: ReactNode | null = null

  if (Array.isArray(props.children)) {
    for (let i = 0; i < props.children.length; i++) {
      if (props.children[i] === For.Each) {
        eachComponent = props.children[i] as ReactWithProp
      } else if (props.children[i] === For.Fallback) {
        fallbackComponent = props.children[i] as ReactNode
      }
    }
  } else {
    eachComponent = props.children as ReactWithProp
  }

  if (eachComponent?.props) {
    const eachProps = eachComponent.props
    if (eachProps?.of) {
      const of = eachProps.of
      if (!of || !of.length) {
