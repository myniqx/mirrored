...
export const MainMenu = () => {
  ...
  const orderedDetails =
    tab === "byIndex"
      ? details
      : nuzulOrder.map((index) => (ayahDetails as AyahDetailsInArray[])[index - 1] as AyahDetailsInArray)
  ...
  const filteredDetails = q ? mappedDetails.filter(({ matchedString }) => matchedString) : mappedDetails

  return (
    <Tabs defaultValue={tab} onValueChange={(value) => changeParams({ tab: value })}>
      <TabsList >
      ...
      <TabsContent value="byIndex" className={`${contentHeight} overflow-y-scroll pr-2`}>
      ...
      <TabsContent value="byNüzul" className={`${contentHeight} overflow-y-scroll pr-2`}>
      ...
      <TabsContent value="byCuz" className={`${contentHeight} overflow-y-scroll pr-2`}>
